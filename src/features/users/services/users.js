...
// Teacher + everything they teach/created (subjects, sections, subject
// allocations, assignments). GET /users/teachers/:id — any logged-in user.
...
// Replace a teacher's full set of subject×section allocations.
...
export async function setTeacherTeaching(id, subjectAllocations) {
  const { data } = await http.put(`/users/teachers/${id}/teaching`, { subjectAllocations });
  return data.data;
}
